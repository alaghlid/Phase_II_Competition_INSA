...
## Cet attaque est conçu pour les équipes qui ne suppriment pas des lignes
###########################################################################################################################
    ...
    for week in weeks:
        ...
        for index, row in df_originale_week.iterrows():
            for index1, row1 in df_anonym_week.iterrows():
                # Egalité des occurences du couple : (id, pseudo_id) => Pseudo_Id est sûrement le guesse de cet Id
                if df_originale_week['latittude']['count'][index] == df_anonym_week['latittude']['count'][index1]:
