...
## Cet attaque est conçu pour les équipes qui modifient rien dans le timestamp (date) et fait round au GPS (longitude - latittude)
###########################################################################################################################
...
def alanozy_reident():
    ...
    for week in weeks:
        ...
        ## Application de la fonction du round
        rounded_cols = ['lat', 'long']
        df_originale_week[rounded_cols] = df_originale_week[rounded_cols].round(
            2)
